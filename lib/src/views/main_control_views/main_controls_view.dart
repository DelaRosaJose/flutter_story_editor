...
class _MainControlsViewState extends State<MainControlsView> {
  ...
  void initState() {
    ...
    for (var file in widget.selectedFiles!) {
      ...
      debugPrint(
          'StoryEditor: Checking file ${file.path}, size: ${sizeInMB.toStringAsFixed(2)} MB');

      if (isVideo(file) && sizeInMB > maxVideoSizeMB) {
        debugPrint(
            'StoryEditor: Video too large (${sizeInMB.toStringAsFixed(2)} MB > $maxVideoSizeMB MB). Popping.');
      ...
      if (!isVideo(file) && sizeInMB > maxImageSizeMB) {
        debugPrint(
            'StoryEditor: Image too large (${sizeInMB.toStringAsFixed(2)} MB > $maxImageSizeMB MB). Popping.');
        WidgetsBinding.instance.addPostFrameCallback((_) {
          Navigator.pop(context);
        });
      }
    ...
    return AnimatedPadding(
      ...
      child: Column(
        children: [
          ...
          // FilterTextView removed to disable filters
          const SizedBox(
            height: 5,
          ),
          Column(
            children: [
              ...
              // FiltersView removed to disable filters
              if (widget.isFocused == false)
                CaptionView(
